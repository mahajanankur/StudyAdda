package com.shivablast.studyadda.web.controller;
...
public class OldRegistrationController {
	...
	public String resendRegistrationToken(final HttpServletRequest request,
			final Model model, @RequestParam("token") final String existingToken) {
		final Locale locale = request.getLocale();
		...
		try {
			final String appUrl = "http://" + request.getServerName() + ":"
					+ request.getServerPort() + request.getContextPath();
			final SimpleMailMessage email = constructResetVerificationTokenEmail(
	...
	public String resetPassword(final HttpServletRequest request,
			final Model model, @RequestParam("email") final String userEmail) {
	...
	public String changePassword(final HttpServletRequest request,
		...
		final PasswordResetToken passToken = userService
		...
		final Authentication auth = new UsernamePasswordAuthenticationToken(
				user, null, userDetailsService.loadUserByUsername(
						user.getEmail()).getAuthorities());
	...
	public String savePassword(final HttpServletRequest request,
			final Model model, @RequestParam("password") final String password) {
		final Locale locale = request.getLocale();
	...
	private final SimpleMailMessage constructResetVerificationTokenEmail(
		...
		final String confirmationUrl = contextPath
				+ "/old/regitrationConfirm.html?token=" + newToken.getToken();
